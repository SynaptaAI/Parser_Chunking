...
from .object_detector import (
    candidate_role_from_segment_type,
    detect_qa_zone,
    detect_text_object,
    detect_title_object,
    extract_numbering,
    is_heading_like_text,
)
        ...
        while i < len(section.blocks):
            ...
            if b.type == "heading":
                ...
                if title_obj_type:
                    ...
                    chunks.append({
                        ...
                        "qa_zone_type": detect_qa_zone(heading_path, title_obj_type),
                        "candidate_role": candidate_role_from_segment_type(title_obj_type),
                        "numbering": extract_numbering(content),
                        "is_heading_like": False,
                        "chapter_main": _chapter_main_from_heading(heading_path),
                    })
                    i = j
                    continue
                ...
                chunks.append({
                    ...
                    "confidence": 1.0,
                    "qa_zone_type": detect_qa_zone(heading_path, heading_seg_type),
                    "candidate_role": "none",
                    "numbering": extract_numbering(b.text.strip()),
            ...
            if b.type in ("table", "image", "formula"):
                ...
                chunks.append({
                    ...
                    "references": extract_references(content),
                    "qa_zone_type": detect_qa_zone(heading_path, b.type),
                    "candidate_role": "none",
                    "numbering": extract_numbering(content),
                    "is_heading_like": False,
                    "chapter_main": _chapter_main_from_heading(heading_path),
            ...
            if _is_list_item(b.text) or _is_procedure_item(b.text):
                ...
                chunks.append({
                    ...
                    "confidence": 1.0,
                    "references": extract_references(content),
                    "qa_zone_type": detect_qa_zone(heading_path, list_kind),
                    "candidate_role": candidate_role_from_segment_type(list_kind),
                    "numbering": extract_numbering(content),
                    "is_heading_like": is_heading_like_text(content),
                    "chapter_main": _chapter_main_from_heading(heading_path),
                })
                i = j
                continue
            ...
            chunks.append({
                ...
                "qa_zone_type": detect_qa_zone(heading_path, obj_type),
                "candidate_role": candidate_role_from_segment_type(obj_type),
                "numbering": extract_numbering(b.text.strip()),
                "is_heading_like": is_heading_like_text(b.text.strip()),
                "chapter_main": _chapter_main_from_heading(heading_path),
            })
            i += 1
...
def build_elements(doc: DocumentTree) -> List[Dict[str, Any]]:
    ...
    def section_page_info(section: SectionNode) -> Tuple[List[int], List[int]]:
        pages: List[int] = []
        if section.start_page >= 0:
            pages.append(section.start_page)
        if section.end_page >= 0:
            pages.append(section.end_page)
        for b in section.blocks:
            pages.append(b.page_idx)
        for child in section.children:
            child_range, child_span = section_page_info(child)
            if child_range:
                pages.extend(child_range)
            if child_span:
                pages.extend(child_span)
        if not pages:
            return [], []
        lo = min(pages)
        hi = max(pages)
    ...
    def emit_section(section: SectionNode) -> bool:
        has_nonempty_block = any(
            (b.type in ("image", "table", "formula")) or bool((b.text or "").strip())
            for b in section.blocks
        )
        has_any_child = bool(section.children)
        if not has_nonempty_block and not has_any_child:
            return False

        heading_path = section.path or section.title
        heading_page_range, heading_page_span = section_page_info(section)
        elements.append({
            "heading_path": heading_path,
            "content": section.title,
            "type": "heading",
            "POS": [],
            "page_range": heading_page_range,
            "page_span": heading_page_span,
            "bbox": None,
            "taxonomy_path": _taxonomy_path(heading_path),
        ...
        for b in section.blocks:
            block_text = b.text or ""
            if b.type not in ("image", "table", "formula") and not block_text.strip():
                continue
            ...
            elements.append({
                ...
                "qa_zone_type": detect_qa_zone(heading_path, seg_type),
                "candidate_role": candidate_role_from_segment_type(seg_type),
                "numbering": extract_numbering(b.text or ""),
                "is_heading_like": is_heading_like_text(b.text or ""),
                "chapter_main": _chapter_main_from_heading(heading_path),
            })
        for child in section.children:
            emit_section(child)
...
def _chapter_main_from_heading(heading_path: str) -> Optional[str]:
    if not heading_path:
        return None
    m = re.search(r'(?:chapter|ch)\s+(\d+)', heading_path, re.IGNORECASE)
    if m:
        return m.group(1)
    m = re.search(r'(^| > )(\d+)(?:\.\d+)*', heading_path)
    if m:
        return m.group(2)
    return None
